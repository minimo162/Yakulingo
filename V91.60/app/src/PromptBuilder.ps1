...
function Get-YakuNumericRulesSection {
    ...
    if ([string]$InputText -notmatch '⟦#[A-Z]{3}⟧|[0-9０-９▲△＋+%％〜~↑↓<>＜＞→]|oku|k units|k yen|YoY|QoQ|CAGR|前年|四半期') { return '' }
    $nl = [Environment]::NewLine
    ...
    if ($Direction -eq 'to_jp') {
        $jpRules = @()
        if ([string]$InputText -match '⟦#[A-Z]{3}⟧') {
            $jpRules += '- NUMBER PLACEHOLDERS (highest priority). Tokens such as ⟦#XXX⟧ and ⟦#YYY⟧ (three capital letters inside ⟦# ⟧) stand for redacted numbers; the three letters are an arbitrary ID, not a word. Copy each token character-for-character, brackets included, into the Japanese output. Never translate, change the letters of, reorder, merge, split, or drop one; never invent one; never replace one with a digit or with a word such as 一定額, 約, 数, or いくつか. Every token in SOURCE appears the same number of times in the output. A number written WITHOUT a placeholder is not redacted: copy it verbatim as a number.'
        }
        ...
        $jpRules += '- Signs follow SOURCE. A value written in parentheses becomes ▲ in front of the value: (⟦#XXX⟧) oku -> ▲⟦#XXX⟧億円. Keep + as +.'
        return (@($jpRules) -join $nl)
    }
    # V91.60: 数値は外部送信前に⟦#ABC⟧へ置き換えている。指示は禁止事項の列挙ではなく
    # 「左に一致したら右を出す」形の決定表で書く。想定外の形が来たときでも
    # 行き先を類推できるようにするため。
    $placeholderRules = @()
    if ([string]$InputText -match '⟦#[A-Z]{3}⟧') {
        $placeholderRules = @(
            '- NUMBER PLACEHOLDERS (highest priority). Tokens such as ⟦#XXX⟧ and ⟦#YYY⟧ (three capital letters inside ⟦# ⟧) stand for redacted numbers; the three letters are an arbitrary ID, not a word. Copy each token character-for-character, brackets included. Never translate, change the letters of, reorder, merge, split, or drop one; never invent one; never replace one with a digit or a word (one, several, approximately, a few). Every token in SOURCE appears the same number of times in each output section. Signs, units, and % stay OUTSIDE the token. A number written WITHOUT a placeholder is not redacted: copy it verbatim as a number.'
            '- Placeholder decision table, SOURCE -> OUTPUT: ⟦#XXX⟧ oku -> ⟦#XXX⟧ oku / +⟦#XXX⟧ oku -> +⟦#XXX⟧ oku / ▲⟦#XXX⟧ oku -> (⟦#XXX⟧) oku / △⟦#XXX⟧% -> (⟦#XXX⟧)% / ⟦#XXX⟧ k units -> ⟦#XXX⟧ k units / ⟦#XXX⟧ k yen -> ⟦#XXX⟧ k yen / ⟦#XXX⟧→⟦#YYY⟧ -> ⟦#XXX⟧→⟦#YYY⟧.'
        )
    }
