...
Excelの既存レイアウトへ収まる訳を作ることが主目的です。確認済みの過去訳は翻訳メモリへ再利用し、同じ表現を何度も作り直さないようにします。Word・テキスト・CSVの互換も保ちますが、製品判断では汎用CAT機能を増やすことよりも、**「元レイアウトを崩さず、その場所に入る訳を作る」精度と手数の少なさ**を優先します。
...
起動画面では、まずExcelの翻訳を選びます。原文セルの幅・結合・周囲の空きセルを読み取り、収まる長さを考慮した候補を作ってから、確認・点検・プレビューを経て新しいファイルへ書き戻します。
