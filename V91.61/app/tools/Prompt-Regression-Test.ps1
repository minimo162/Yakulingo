...
Assert-YakuTrue -Condition ($briefRules.Contains('sales promotion costs / promotion costs -> VM') -and $briefRules.Contains('fixed sales promotion costs -> Fixed MKT') -and $briefRules.Contains('never write Promo. Costs')) -Message 'V91.57 BRIEF promotion-cost rules missing'
Assert-YakuTrue -Condition ($textTemplate.Contains('FULL also uses sales promotion costs, fixed sales promotion costs') -and $textTemplate.Contains('Do not write Promo. Costs or Fixed Promo. in either mode')) -Message 'V91.57 FULL promotion-cost expansion rule missing'
Assert-YakuTrue -Condition ($fileTemplate.Contains('子会社 固定販促費 / 子会社固定販促費 -> Subs. Fixed MKT') -and $fileTemplate.Contains('販売奨励金/固定販促費 -> VM / Fixed MKT') -and $fileTemplate.Contains('国内その他 -> Dom. Oth.') -and $fileTemplate.Contains('連結調整他 -> Cons. Adj.')) -Message 'V91.58 shortened file exact-match labels missing'
...
Assert-YakuTrue -Condition ($v9157PromptGlossary.Contains('固定販促費,Fixed MKT') -and $v9157PromptGlossary.Contains('販促費,VM') -and $v9157PromptGlossary.Contains('子会社 固定販促費,Subs. Fixed MKT')) -Message 'V91.57 prompt glossary promotion-cost mappings missing'
Assert-YakuTrue -Condition ($v9157MainGlossary.Contains('固定販促費,Fixed MKT') -and $v9157MainGlossary.Contains('子会社 固定販促費,Subs. Fixed MKT') -and $v9157MainGlossary.Contains('子会社固定販促費,Subs. Fixed MKT') -and $v9157MainGlossary.Contains('販売奨励金/固定販促費,VM / Fixed MKT') -and $v9157MainGlossary.Contains('国内その他,Dom. Oth.') -and $v9157MainGlossary.Contains('連結調整他,Cons. Adj.')) -Message 'V91.58 exact-match shortened mappings missing'
# 変動側 VM / 固定側 Fixed MKT の対。Promo. 系へ戻っていないことを見る（利用者の指示 2026-08-05）。
Assert-YakuTrue -Condition (-not $v9157MainGlossary.Contains('Fixed Promo.') -and -not $v9157MainGlossary.Contains('販売促進費,Promo. Costs')) -Message 'V91.61 superseded Promo. promotion-cost mappings remain'
