...
.DESCRIPTION
  ...
  (b) 実装地図の部。Server.ps1 から本物の関数を AST で取り出し、
      その場で呼ぶ（写経しない）。
        ...
        - 曖昧な原文でも 409 にならないこと
        - /api/palette/translate が Start-YakuTranslationJob を
          -Kind 'text' で呼ぶこと。実行中ジョブの throw を 409/JOB_RUNNING
          へ写すこと
        - /api/palette/chip が 'shorten'→Kind shorten、'revise'→Kind revise
          で呼ぶこと。current_text に渡した値がそのまま ReviseJson へ
          乗ること（マスクを外さない配線になっているか）
  ...
  (c) 実機Chromium部。本物の palette.html/palette.js を配り、貼り付け→
      即答表示→ジョブ完了→1〜9キーでコピー→チップ→Escで消去、を
      実際に押して確かめる。node/Playwright/Chromium が無い環境では
...
Assert-N9195 ($script:N9195StartJobCalls.Count -eq 1 -and [string]$script:N9195StartJobCalls[0].Kind -eq 'shorten') '「短く」チップが Start-YakuTranslationJob を -Kind shorten で呼ぶ'
$N9195ShortenReviseJson = $script:N9195StartJobCalls[0].ReviseJson | ConvertFrom-Json
Assert-N9195 ([string]$N9195ShortenReviseJson.current_text -eq 'Revenue increased by [[N1]] percent.') '「短く」チップの current_text は渡した値のまま(マスクを外さない)'
...
$script:N9195StartJobCalls.Clear()
$N9195ChipShortenJp = Invoke-N9195Route -Method 'POST' -Path '/api/palette/chip' -Payload @{
    chip = 'shorten'; source_text = 'x'; current_text = 'y'; direction = 'to_jp'; style = 'full'
}
Assert-N9195 ($N9195ChipShortenJp.Status -eq 400 -and $script:N9195StartJobCalls.Count -eq 0) '「短く」は和訳(to_jp)では使えない(Invoke-YakuTextShortenはto_en決め打ちのため)'
...
# --- Convert-YakuTranslationJobResultJson: source_text/masked_translation の配線 ---
...
Assert-N9195 ([string]$N9195ResultJson.masked_translation -eq 'Real value [[N1]].') 'Convert-YakuTranslationJobResultJson が masked_translation(実値なし)を返す'
Assert-N9195 ([string]$N9195ResultJson.direction -eq 'to_en') 'Convert-YakuTranslationJobResultJson が direction を返す'
Assert-N9195 ($N9195ResultJson.html -match 'masking-note') 'Convert-YakuTranslationJobResultJson の html にマスク件数の表示が入っている'
...
if (Test-Path -LiteralPath $N9195OutJson -PathType Leaf) {
    ...
    Assert-N9195 (@($N9195Observed.console).Count -eq 0) '実機ページでconsole.errorが出ない'
    ...
    Assert-N9195 ($N9195Observed.shortenChipHidden -eq $false) '英訳では「短く」チップが出る'
    ...
    Assert-N9195 ($N9195AfterThree.Count -eq 3 -and [string]$N9195AfterThree[2] -eq 'Sales reached 1,234 million yen.') '3キーでCopilotの添え(alt)候補がコピーされる'
    ...
    if ($N9195ChipReq.Count -eq 1) {
        ...
        Assert-N9195 ([string]$N9195ChipReq[0].current_text -eq 'Revenue was [[N1]] million yen.') 'チップ送信の current_text はマスク後の値(画面表示の実値入り文字列ではない)'
