...
try {
    foreach ($name in @('Paths.ps1','Runtime.ps1','Html.ps1','Settings.ps1','PromptBuilder.ps1','BriefStyle.ps1','EdgeLaunch.ps1','CopilotBudget.ps1','CopilotClient.ps1','Translation.ps1','FileProcessors.ps1','FileTranslation.ps1','CellSegments.ps1','CellAlign.ps1','CatProject.ps1')) {
    ...
    $style = Get-YakuCatCacheStyle -CorpusSection ''
    ...
    $callback = {
        ...
        if ($rows.Count -gt 0) { $null = Save-YakuCatBatchCheckpoint -ProjectId $project.Id -Translations $rows }
    }.GetNewClosure()
    $context = @{
        BatchOrdinal=0; TotalBatches=2; MaxRetryDepth=0; CorpusSection=''
    ...
    $threw = $false
    try {
        $null = Invoke-YakuFileTranslationItems -Root $root -Items @($items.ToArray()) -Settings $settings -Direction 'to_en' -MaxChars 300 -Warnings $warnings -Context $context
    } catch { $threw = $true }
    Chk ($threw -and $script:YakuRecoverySends -eq 2) '1バッチ成功後の強制失敗を再現する'
    if ($context['CompletedMap'].Count -eq 0) {
        Write-Host ('  diagnostic completed=0 warnings=' + (@($warnings | ForEach-Object { [string]$_.Message }) -join ' | ')) -ForegroundColor Yellow
    }
    ...
    Chk ([int]$usage.CacheHits -eq 1 -and [int]$usage.EstimatedCalls -eq 0) '成功バッチのcache writer結果をCAT readerが再利用する'
