...
function Convert-YakuSettingsFormToHtml {
    param([Parameter(Mandatory=$true)]$Settings)
    $bundled = if ($Settings.use_bundled_glossary) { 'checked' } else { '' }
    $csvHeader = if ($Settings.csv_translate_header) { 'checked' } else { '' }
    $shapes = if ($Settings.translate_shapes) { 'checked' } else { '' }
    $charts = if ($Settings.translate_charts) { 'checked' } else { '' }
    $cache = if ($Settings.translation_cache_enabled) { 'checked' } else { '' }
    $diagnosticsLevel = Get-YakuDiagnosticsLevel -Settings $Settings
    $diagnosticsValue = $diagnosticsLevel
    $diagnosticsLabel = "ログ診断レベル：$diagnosticsLevel（保存済み）"
    $diagnosticsClass = if ($diagnosticsLevel -eq 'full') { 'is-enabled' } else { 'is-disabled' }
    $diagMinimalSelected = if ($diagnosticsLevel -eq 'minimal') { 'selected' } else { '' }
    $diagStandardSelected = if ($diagnosticsLevel -eq 'standard') { 'selected' } else { '' }
...
<form class='settings-grid' data-yaku-json-post='/api/settings' data-yaku-target='#settings-result' data-yaku-settings-form='true' data-yaku-dirty='false' data-yaku-saved-diagnostics='$diagnosticsValue'>
  ...
  <label>ファイル抽出タイムアウト秒 <input type='number' min='60' max='3600' name='extract_timeout_seconds' value='$(ConvertTo-YakuHtml $Settings.extract_timeout_seconds)'></label>
  <label>テキスト翻訳バッチ文字数 <input type='number' min='0' max='20000' name='max_chars_per_batch' value='$(ConvertTo-YakuHtml $Settings.max_chars_per_batch)'></label>
  <label>ファイル翻訳バッチ文字数 <input type='number' min='300' max='20000' name='max_chars_per_batch_file' value='$(ConvertTo-YakuHtml $Settings.max_chars_per_batch_file)'></label>
  <label>用語集プロンプト上限 <input type='number' min='1' max='200' name='glossary_prompt_limit' value='$(ConvertTo-YakuHtml $Settings.glossary_prompt_limit)'></label>
