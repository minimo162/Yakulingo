...
V91.61（2026-08-06）で、用語集は **`glossary.csv` の1本だけ**になりました。用途は Excel/CSV のセルが完全一致したときの置換で、目的は**列幅に収めること（レイアウトの保証）**です。

文中の言い回しの統一には用語集を使いません。プロンプトへの用語集注入（`prompt_glossary.csv`）と、訳文が用語集どおりかを見る監査は廃止しました。文中は参考資料コーパスの文例で寄せます。

理由は `_docs/実機検証結果_2026-08-05.md` にあります。文中では用語集がコーパスの言い回しを打ち消していたこと、そして既存の翻訳製品も文中の用語を機械的に置換していない（CAT/TMS は印を付けて人が直し、DeepL は「search-and-replace 方式ではない」と明記、Google は用語集適用前後の両方を返す）ことによります。活用と一致が壊れるためです。

用語の追加・削除はアプリからは行えません。`glossary.csv` を直接編集してください。件数、パス、重複・競合警告、全エントリは用語集パネルで確認できます。
...
- テキスト: 日本語はFULL/BRIEFの英訳、英語・その他は日本語訳
- ファイル: `.xlsx`、`.xlsm`、`.csv`
...
- ファイル翻訳は専用PowerShellプロセスと専用Excelインスタンスで実行します。サーバーはジョブID別状態取得とキャンセルへ応答し続けます。
...
| 画面内履歴 | 現在のサーバープロセス内に最大10件。原文・訳文をディスクへ保存しない |
| `history\history.jsonl` | ハッシュ、文字数、件数のみ。最大1MB、既定30日 |
