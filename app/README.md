...
Microsoft 365 Copilotの画面をEdge DevTools Protocol（CDP）で操作するローカル業務ツールです。起動時はテキスト翻訳を表示し、原文と訳文の2ペインから明示的に翻訳・短縮・コピーできます。Excel翻訳は上部切替から開き、確認済み対訳の再適用、未訳補完、QC、非破壊出力に限定します。配置・体裁調整、publication版、プレビュー/PDF、一括収まり判定、inspectorは扱いません。Copilotへ送る前の数値マスクは両機能で維持します。
...
前回版は利用者がその作業へ明示的に読み込んだ場合だけ、「前回版」と出所を明示して候補表示します。訳文への自動反映や翻訳メモリへの自動昇格はしません。テキスト翻訳は、用語集・翻訳メモリ・前回版を参照も保存もしません。
