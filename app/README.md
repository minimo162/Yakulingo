...
Microsoft 365 Copilotの画面をEdge DevTools Protocol（CDP）で操作するローカル業務ツールです。起動時はテキスト翻訳を表示し、原文と訳文の2ペインから明示的に翻訳・短縮・コピーできます。Excel翻訳は上部切替から開き、確認済み対訳の再適用、未訳補完、QC、非破壊出力を行います。Copilotへ送る前の数値マスクは両機能で維持します。
