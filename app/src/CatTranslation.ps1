...
function Copy-YakuCatPipelineItem { param($Item);[Management.Automation.PSSerializer]::Deserialize([Management.Automation.PSSerializer]::Serialize($Item,20)) }
function New-YakuCatPipelineData { param([object[]]$Items);$r=New-Object Collections.Generic.List[object];foreach($i in $Items){$x=[ordered]@{item=[int]$i.Index};foreach($n in @('PipelineDraft','PipelineCandidates','PipelineClaims','PipelineSelected')){if($i.PSObject.Properties.Name -contains $n){$x[$n]=[string]$i.$n}};if($null -ne $i.MaxChars){$x.max_chars=[int]$i.MaxChars};$r.Add($x)|Out-Null};@($r.ToArray())|ConvertTo-Json -Compress -Depth 5 }
...
function Invoke-YakuCatTranslationItems {
 ...
 if($over.Count){$lists=@{};$metas=@{};foreach($i in $over){$lists[[int]$i.Index]=New-Object Collections.Generic.List[string];$metas[[int]$i.Index]=New-Object Collections.Generic.List[object]};$n=3;try{$n=[Math]::Max(1,[Math]::Min(5,[int]$Settings.cat_fit_candidate_count))}catch{}
  for($r=0;$r-lt$n;$r++){$q=New-Object Collections.Generic.List[object];foreach($i in $over){$x=Copy-YakuCatPipelineItem $i;$x|Add-Member PipelineStage compress -Force;$x|Add-Member PipelineDraft ([string]$draft[[int]$i.Index]) -Force;$q.Add($x)|Out-Null};$cm=Invoke-YakuCatPipelineBatch $Root @($q.ToArray()) $Settings to_en $MaxChars $Warnings $ProgressState $Context;foreach($i in $over){$id=[int]$i.Index;if($cm.ContainsKey($id)){$z=Split-YakuCatCompressionResult ([string]$cm[$id]);$metas[$id].Add($z)|Out-Null;if((Test-YakuCatCompressionCandidate $i $z.Translation)-and-not$lists[$id].Contains($z.Translation)){$lists[$id].Add($z.Translation)|Out-Null}}}}
  $q=New-Object Collections.Generic.List[object];foreach($i in $over){$id=[int]$i.Index;if(-not$lists[$id].Count){continue};$x=Copy-YakuCatPipelineItem $i;$x|Add-Member PipelineStage select -Force;$x|Add-Member PipelineDraft ([string]$draft[$id]) -Force;$x|Add-Member PipelineCandidates ((@($lists[$id].ToArray())|ConvertTo-Json -Compress)) -Force;$q.Add($x)|Out-Null};if($q.Count){$sm=Invoke-YakuCatPipelineBatch $Root @($q.ToArray()) $Settings to_en $MaxChars $Warnings $ProgressState $Context;foreach($i in $over){$id=[int]$i.Index;if($sm.ContainsKey($id)-and(Test-YakuCatCompressionCandidate $i ([string]$sm[$id]))){$final[$id]=[string]$sm[$id]}elseif($lists[$id].Count){$final[$id]=$lists[$id][0]}}}
  $q=New-Object Collections.Generic.List[object];foreach($i in $over){$id=[int]$i.Index;if(-not$final.ContainsKey($id)){continue};$x=Copy-YakuCatPipelineItem $i;$x.Text=[string]$final[$id];$x.MaskedText=$x.Text;$x|Add-Member PipelineStage back_reconstruct -Force;$x|Add-Member PipelineSelected $x.Text -Force;$q.Add($x)|Out-Null};$retry=@{};if($q.Count){$cl=Invoke-YakuCatPipelineBatch $Root @($q.ToArray()) $Settings to_jp $MaxChars $Warnings $ProgressState $Context;$j=New-Object Collections.Generic.List[object];foreach($i in $over){$id=[int]$i.Index;if(-not$cl.ContainsKey($id)){continue};$x=Copy-YakuCatPipelineItem $i;$x|Add-Member PipelineStage back_judge -Force;$x|Add-Member PipelineClaims ([string]$cl[$id]) -Force;$x|Add-Member PipelineSelected ([string]$final[$id]) -Force;$j.Add($x)|Out-Null};if($j.Count){$jm=Invoke-YakuCatPipelineBatch $Root @($j.ToArray()) $Settings to_en $MaxChars $Warnings $ProgressState $Context;foreach($i in $over){$id=[int]$i.Index;if($jm.ContainsKey($id)-and[string]$jm[$id]-match'^RETRY_REQUIRED:'){$retry[$id]=$true}}}}
  if($retry.Count){$q=New-Object Collections.Generic.List[object];foreach($i in $over){$id=[int]$i.Index;if(-not$retry.ContainsKey($id)){continue};$x=Copy-YakuCatPipelineItem $i;$x|Add-Member PipelineStage compress -Force;$x|Add-Member PipelineDraft ([string]$draft[$id]) -Force;$q.Add($x)|Out-Null};$rm=Invoke-YakuCatPipelineBatch $Root @($q.ToArray()) $Settings to_en $MaxChars $Warnings $ProgressState $Context;foreach($i in $over){$id=[int]$i.Index;if($rm.ContainsKey($id)){$z=Split-YakuCatCompressionResult ([string]$rm[$id]);if(Test-YakuCatCompressionCandidate $i $z.Translation){$final[$id]=$z.Translation}}}}
 ...
 $Context.FitOverflows=$fo;if($Context.ContainsKey('CompletedMap')){foreach($k in $final.Keys){$Context.CompletedMap[[int]$k]=$final[$k]}};if($Context.OnBatchCompleted){& $Context.OnBatchCompleted $Items $final};$final
}
