...
.DESCRIPTION
  ...
  オフライン継続・manifestなしの直接起動・ローカル破損修復・manifest境界を検証する。
...
if ([string]::IsNullOrWhiteSpace($PackageRoot)) { $PackageRoot = Split-Path -Parent $appRoot }
if ([string]::IsNullOrWhiteSpace($BootstrapPath)) { $BootstrapPath = Join-Path $PackageRoot 'bootstrap.ps1' }
...
function Copy-YakuPublishedTree {
    ...
    Copy-Item -LiteralPath (Join-Path $Source 'app') -Destination (Join-Path $Destination 'app') -Recurse -Force
}
...
try {
    ...
    Copy-YakuPublishedTree -Source $PackageRoot -Destination $shared
    $newPackage = Join-Path $shared 'app\tools\New-YakuPackage.ps1'
    & $newPackage -SourceRoot $shared -OutputPath (Join-Path $sandbox 'pkg.zip') -BuildId $buildId | Out-Null

    Write-Host 'CASE 0: package生成側も危険なbuild_idを拒否する'
    $buildBefore = [IO.File]::ReadAllText((Join-Path $shared 'app\config\build.txt'))
    Assert-YakuBootstrapThrows { & $newPackage -SourceRoot $shared -BuildId '..\escape' -ManifestOnly } 'PACKAGE_BUILD_ID_INVALID' 'パスへ使えないBuildIdをmanifest生成前に拒否する'
    Assert-YakuBootstrap ([IO.File]::ReadAllText((Join-Path $shared 'app\config\build.txt')) -eq $buildBefore) '不正BuildIdではbuild.txtを書き換えない'

    Write-Host 'CASE 1: 初回導入'
    $run1 = & $BootstrapPath -SharedRoot $shared -LocalRoot $local -NoLaunch
    Assert-YakuBootstrap ($run1 -like (Join-Path (Join-Path $local 'versions') ($buildId + '-*'))) 'ローカルへ導入される'
    ...
    $sharedVictim = Join-Path $shared 'app\prompts\text_translate_full_to_en.txt'
    ...
    Add-Content -LiteralPath (Join-Path $shared 'app\DESIGN.md') -Value "`nbootstrap update probe"
    & $newPackage -SourceRoot $shared -OutputPath (Join-Path $sandbox 'pkg2.zip') -BuildId $buildId | Out-Null
    ...
    Assert-YakuBootstrapThrows { & $BootstrapPath -SharedRoot $brokenShared -LocalRoot (Join-Path $sandbox 'local-empty') -NoLaunch } 'SHARED_PACKAGE_NOT_FOUND' '共有もローカルも無ければ停止する'
    ...
    Assert-YakuBootstrapThrows { & $BootstrapPath -SharedRoot $brokenShared -LocalRoot $offlineCorrupt -NoLaunch } 'SHARED_PACKAGE_NOT_FOUND' '共有が無いとき改変済みローカル版へフォールバックしない'
    ...
    $victim = Join-Path $tampered 'app\prompts\text_translate_full_to_en.txt'
    ...
    $unsafePathManifestPath = Join-Path $unsafePathShared 'manifest.json'
    ...
    $unsafeBuildManifestPath = Join-Path $unsafeBuildShared 'manifest.json'
    ...
    $duplicateManifestPath = Join-Path $duplicateShared 'manifest.json'
    ...
    $badShaManifestPath = Join-Path $badShaShared 'manifest.json'
    ...
    Write-Host 'CASE 6: manifest の無いフラット配置は共有フォルダ上で直接起動する'
    $legacyShared = Join-Path $sandbox 'shared-legacy'
    Copy-Item -LiteralPath $shared -Destination $legacyShared -Recurse -Force
    Remove-Item -LiteralPath (Join-Path $legacyShared 'manifest.json') -Force
    $run6 = & $BootstrapPath -SharedRoot $legacyShared -LocalRoot (Join-Path $sandbox 'local-legacy') -NoLaunch
    Assert-YakuBootstrap ($run6 -eq $legacyShared) '共有ルート直下の app を直接起動する'

    Write-Host 'CASE 7: 旧バージョンフォルダだけの共有ルートは起動対象にしない'
    $nestedOnly = Join-Path $sandbox 'shared-nested-only'
    New-Item -ItemType Directory -Path (Join-Path $nestedOnly 'V99.99\app') -Force | Out-Null
    [IO.File]::WriteAllText((Join-Path $nestedOnly 'V99.99\app\Start-YakuLingo.ps1'), "param()`r`n", (New-Object Text.UTF8Encoding($true)))
    Assert-YakuBootstrapThrows { & $BootstrapPath -SharedRoot $nestedOnly -LocalRoot (Join-Path $sandbox 'local-nested-only') -NoLaunch } 'SHARED_PACKAGE_NOT_FOUND' '旧バージョンフォルダを自動選択しない'
