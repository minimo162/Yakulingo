...
**ケース単位の差は読まないこと。** ±0.14 動くので、A/B で見えた ±0.05〜0.08 の増減は
ほぼ意味がない。改善/悪化の印は 0.03 のしきい値で付けているが、
**この印はケース単位では信用できない。グループ平均で判断すること。**

差が小さいときはケース数を増やす。n=4 では1件の外れ値が結論を左右する。
